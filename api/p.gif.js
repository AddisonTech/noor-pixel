...
export default async function handler(req, res){
  try {
    const d = req.query.d ? JSON.parse(req.query.d) : {};
    const ev = normalize(req, d);
    console.log("noor_event", ev);
    if (isGoodLead(ev)) await sendToMeta(ev);
  } catch(e){}
  const gif = Buffer.from("R0lGODlhAQABAPAAAP8AAP///yH5BAAAAAAALAAAAAABAAEAAAICRAEAOw==","base64");
